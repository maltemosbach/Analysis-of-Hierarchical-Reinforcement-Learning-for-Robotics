...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        if hparams["modules"][self.layer_number] == "ddpg":
            self.policy = DDPG(self.sess, env, hparams, self.batch_size, self.experience_buffer, self.replay_buffer, self.transitions_buffer, self.erb, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, buffer_type=hparams["buffer"][self.layer_number], action_l2=self.action_l2)
            self.critic = None
            self.actor = None
        ...
        while True:
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.layer_number) or agent.steps_taken >= env.max_actions or attempts_made >= self.time_limit:
                ...
                if self.return_to_higher_level(max_lay_achieved, agent, env, attempts_made):
                    ...
                    if not agent.FLAGS.test and (self.hparams["buffer"][self.layer_number] == "transitions" or self.hparams["buffer"][self.layer_number] == "replay" or self.hparams["buffer"][self.layer_number] == "erb"):
                        obs.append(o.copy())
                        achieved_goals.append(ag.copy())
        ...
        if self.hparams["modules"][self.layer_number] == "ddpg":
            ...
            if self.hparams["buffer"][self.layer_number] == "experience":
                ...
                if self.hparams["use_target"][self.layer_number]:
                    for _ in range(num_updates):
                        # Update weights of non-target networks
                        if self.erb.get_current_size() > 0:
                    ...
                    for _ in range(num_updates):
                        # Update weights of non-target networks
                        if self.transitions_buffer.get_current_size() > 0:
                            transitions = self.erb.sample(self.batch_size)
                            old_states = transitions['o']
                            actions = transitions['u']
                            rewards = transitions['r']
