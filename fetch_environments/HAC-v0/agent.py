        ...
        if log:
            ...
            if self.hparams["layers"] > 1:
                ...
                if self.hparams["modules"][1] == "actorcritic":
                    ...
                    # Logging exemplary weight to verify transfer learning
                    self.writer.add_histogram('layer_1/critic_weights_main_1', self.layers[1].critic.weights_infer_1[0].eval(session=self.sess), step)
                    self.writer.add_histogram('layer_1/critic_weights_main_2', self.layers[1].critic.weights_infer_2[0].eval(session=self.sess), step)
                    self.writer.add_histogram('layer_1/critic_weights_target_1', self.layers[1].critic.weights_target_1[0].eval(session=self.sess), step)
                    self.writer.add_histogram('layer_1/critic_weights_target_2', self.layers[1].critic.weights_target_2[0].eval(session=self.sess), step)
                    self.writer.add_histogram('layer_1/actor_weights_main', self.layers[1].actor.weights[0].eval(session=self.sess), step)
                    self.writer.add_histogram('layer_1/actor_weights_target', self.layers[1].actor.target_weights[0].eval(session=self.sess), step)
