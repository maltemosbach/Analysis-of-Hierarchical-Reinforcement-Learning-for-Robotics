...
class DDPGL1():
    ...
    def __init__(self, sess, env, hparams, batch_size, experience_buffer, transitions_buffer, layer_number, FLAGS, hidden, layers, T, buffer_type='transitions', Q_lr=0.001, pi_lr=0.001, tau=0.05, 
        gamma=0.98, action_l2=0.0, norm_eps=0.01, norm_clip=5, clip_obs=200):
        ...
        self.sess = sess
        self.scope = "DDPGL1_layer_" + str(layer_number)
        ...
        self.gamma = gamma
        self.polyak = 1-tau
        self.clip_return = FLAGS.time_scale
        ...
        stage_shapes['r'] = (None,)
        stage_shapes['is_t'] = (None,)
        self.stage_shapes = stage_shapes
        print("stage_shapes for DDPGL1:", self.stage_shapes)
    ...
    def sample_batch_experience_buffer(self, update_stats=True):
        if self.experience_buffer.size >= self.batch_size:
            ...
            assert False, "Sample_batch should only be called with enough transitions in the experience_buffer"
    ...
    def sample_batch_transitions_buffer(self, update_stats=True):
        ...
        transitions['is_t'] = transitions['is_t'].astype(int)
        #print("transitions:", transitions)
    ...
    def _create_network(self, input_dims, name=None):
        ...
        target_tf = tf.clip_by_value(batch_tf['r'] + (1-batch_tf['is_t']) * (self.gamma * target_Q_pi_tf), *clip_range)
...
class ActorCritic:
    ...
    def __init__(self, inputs_tf, dimo, dimg, dimu, max_u, o_stats, g_stats, hidden, layers,
        ...
        with tf.variable_scope('Q'):
            ...
            self.Q_pi_tf = tf.sigmoid( self.q_offset + nn(input_Q, [self.hidden] * self.layers + [1]) ) * self.q_limit
            # for critic training
            input_Q = tf.concat(axis=1, values=[o, g, self.u_tf / self.max_u])
            self._input_Q = input_Q  # exposed for tests
            self.Q_tf = tf.sigmoid( self.q_offset + nn(input_Q, [self.hidden] * self.layers + [1], reuse=True) ) * self.q_limit
