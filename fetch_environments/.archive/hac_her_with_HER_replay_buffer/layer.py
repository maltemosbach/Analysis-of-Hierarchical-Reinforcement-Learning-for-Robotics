...
from actorANDcritic import ActorAndCritic
import config
...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        self.state_normalizer = normalizer(size=env.state_dim, eps=0.001, default_clip_range=5)
        self.goal_normalizer = normalizer(size=env.end_goal_dim, eps=0.001, default_clip_range=5)
        ...
        self.critic = Critic(sess, writer, env, self.layer_number, FLAGS, self.state_normalizer, self.goal_normalizer, hparams)
        self.actor = Actor(sess, writer, env, self.batch_size, self.layer_number, FLAGS, self.state_normalizer, self.goal_normalizer, hparams)
        #ddpg_params = {'norm_clip': 5, 'pi_lr': 0.001, 'action_l2': 1.0, 'rollout_batch_size': 1, 'bc_loss': 0, 'batch_size': 256, 'num_demo': 100, 'clip_pos_returns': True, 'sample_transitions': "Test", 'gamma': 0.98, 'clip_return': 49.99999999999996, 'Q_lr': 0.001, 'q_filter': 0, 'clip_obs': 200.0, 'T': 50, 'buffer_size': 1000000, 'hidden': 256, 'max_u': 1.0, 'relative_goals': False, 'prm_loss_weight': 0.001, 'info': {'env_name': 'FetchReach-v1'}, 'layers': 3, 'scope': 'ddpg', 'network_class': 'baselines.her.actor_critic:ActorCritic', 'subtract_goals': "Test2", 'demo_batch_size': 128, 'polyak': 0.95, 'aux_loss_weight': 0.0078, 'norm_eps': 0.01, 'input_dims': {'info_is_success': 1, 'g': 3, 'u': 4, 'o': 10}}
        #self.ddpg = DDPG(reuse=False, **ddpg_params, use_mpi=True)


        self.hidden = 256
        self.layers = 3
        #print('self.env.gymEnv.sim.model.__dict__:', self.env.gymEnv.sim.model.__dict__)
        self.sample_transitions = config.configure_her(self.env.gymEnv)
        self.aAndc = ActorAndCritic(self.sess, self.writer, env, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.sample_transitions)        
    ...
    def choose_action(self,agent, env, subgoal_test):
        ...
        if agent.FLAGS.test or subgoal_test:
            #print("original action:", self.actor.get_target_action(np.reshape(self.current_state,(1,len(self.current_state))), np.reshape(self.goal,(1,len(self.goal))))[0])
            #print("self.aAndc.get_actions(o, g):", self.aAndc.get_actions(o, g))
            ...
            return self.aAndc.get_actions(o, g), "Policy", subgoal_test
            ...
            if np.random.random_sample() > 0.3:
                ...
                action = self.add_noise(self.aAndc.get_actions(o, g), env)
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        self.input_dims = {"o" : env.state_dim, "g" : env.end_goal_dim, "u" : env.action_dim}
         # compute observations
        o = np.empty((1, self.dims['o']), np.float32)  # observations
        ag = np.empty((1, self.dims['g']), np.float32)  # achieved goals
        o[:] = self.current_state
        ag[:] = env.project_state_to_end_goal(env.sim, o)
            ...
            # If layer is bottom level, execute low-level action
                ...
                if agent.steps_taken == 49:
                    ...
                    episode = dict(o=obs,
                       ...
                       g=goals,
                       ag=achieved_goals)
                    print('Storing the finished episode')
                    aAndc.storeEpisode(convert_episode_to_batch_major(episode))
    ...
    def learn(self, num_updates):
        ...
        #print("layer.learn() is called with %d num_updates and replay_buffer.size = %d" % (num_updates, self.replay_buffer.size))
        '''
        for _ in range(num_updates):
            # Update weights of non-target networks
            if self.replay_buffer.size >= self.batch_size:
