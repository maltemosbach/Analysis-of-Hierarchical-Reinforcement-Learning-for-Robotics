...
class TD3():
    ...
    def _create_network(self, input_dims, name=None):
        logger.info("Creating a TD3 agent with max_u {max_u} and u_offset {u_offset} ...".format(max_u=self.max_u, u_offset=self.u_offset))
        ...
        self.Q_1_loss_tf = tf.reduce_mean(tf.square(tf.stop_gradient(target_tf) - self.main.Q_1_tf))
        self.Q_2_loss_tf = tf.reduce_mean(tf.square(tf.stop_gradient(target_tf) - self.main.Q_2_tf))
        # Actor loss is calculated on the basis of Q_1
        self.pi_loss_tf = -tf.reduce_mean(self.main.Q_1_pi_tf)
        # action_l2 term is added to the actor-loss
        ...
        self.Q_grads_vars_tf = zip(Q_grads_tf, self._vars('main/Q_1') + self._vars('main/Q_2'))
        self.pi_grads_vars_tf = zip(pi_grads_tf, self._vars('main/pi'))
        self.Q_grad_tf = flatten_grads(grads=Q_grads_tf, var_list=self._vars('main/Q_1') + self._vars('main/Q_2'))
        self.pi_grad_tf = flatten_grads(grads=pi_grads_tf, var_list=self._vars('main/pi'))

        # optimizers
        self.Q_adam = MpiAdam(self._vars('main/Q_1') + self._vars('main/Q_2'), scale_grad_by_procs=False)
        self.pi_adam = MpiAdam(self._vars('main/pi'), scale_grad_by_procs=False)

        # polyak averaging
        self.main_vars = self._vars('main/Q_1') + self._vars('main/Q_2') + self._vars('main/pi')
        self.target_vars = self._vars('target/Q_1') + self._vars('target/Q_2') + self._vars('target/pi')
    ...
    def _grads(self):
        ...
        if self.total_it % self.policy_freq == 0:
            critic_loss, actor_loss, Q_grad, pi_grad, Q_1_loss, Q_2_loss = self.sess.run([
                self.Q_loss_tf,
                self.main.Q_1_pi_tf,
                self.Q_grad_tf,
                self.pi_grad_tf,
            ...
            critic_loss, Q_grad, Q_1_loss, Q_2_loss = self.sess.run([
                self.Q_loss_tf,
                self.Q_grad_tf,
                self.Q_1_loss_tf,
                self.Q_2_loss_tf
        ...
        self.Q_1_loss = Q_1_loss
        self.Q_2_loss = Q_2_loss
...
class ActorCritic:
    ...
    def __init__(self, inputs_tf, dimo, dimg, dimu, max_u, o_stats, g_stats, hidden, layers,
        ...
        with tf.variable_scope('Q_2'):
            # for policy training
            input_Q = tf.concat(axis=1, values=[o, g, self.pi_tf / self.max_u])
            self.Q_2_pi_tf = nn(input_Q, [self.hidden] * self.layers + [1])
            # for critic training
            input_Q = tf.concat(axis=1, values=[o, g, self.u_tf / self.max_u])
            self._input_Q = input_Q  # exposed for tests
