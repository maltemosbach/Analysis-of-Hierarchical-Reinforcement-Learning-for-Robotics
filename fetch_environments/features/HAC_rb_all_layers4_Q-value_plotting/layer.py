...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        # Check which buffer ddpg should use
        if self.layer_number == 0 and (hparams["use_rb"] == "lowest" or hparams["use_rb"] == "all"):
            self.use_replay_buffer = True
        elif self.layer_number > 0 and hparams["use_rb"] == "all":
            self.use_replay_buffer = True
        else:
            self.use_replay_buffer = False

        if hparams["algo"] == "ddpg":
            self.policy = DDPG(self.sess, self.writer, env, hparams, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, use_replay_buffer=self.use_replay_buffer, action_l2=self.action_l2)  
        elif hparams["algo"] == "td3":
            self.policy = TD3(self.sess, self.writer, env, hparams, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, use_replay_buffer=self.use_replay_buffer, action_l2=self.action_l2)  
        else:
            assert False
    ...
    def append_transitions_rb(self, env, o, u, ag, g, obs, achieved_goals, acts, goals, info_is_sgtt, layer_number, goal_status, highest_layer, is_sgtt):

        o_new = env.obs['observation']
        if layer_number == highest_layer:
            ag_new = env.project_state_to_end_goal(o_new)
        else:
            ag_new = env.project_state_to_subgoal(o_new)
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        if self.layer_number == self.FLAGS.layers-1:
            ag[:] = env.project_state_to_end_goal(self.current_state)
        else:
            ag[:] = env.project_state_to_subgoal(self.current_state)
        ...
        while True:
            ...
            #    print("next_subgoal_test:", next_subgoal_test)
            u = action
            if u.ndim == 1:
                u = u.reshape(1, -1)
            ...
            if self.layer_number > 0:
                ...
                #print("Layer 1 propses subgoal:", action)
            ...
            if not agent.FLAGS.test:
                ...
                if next_subgoal_test:
                    ...
                    if agent.layers[self.layer_number-1].maxed_out:
                        self.append_transitions_rb(env, o, hindsight_u, ag, self.g, obs, achieved_goals, acts, goals, info_is_sgtt, self.layer_number, goal_status, self.FLAGS.layers-1, is_sgtt=-1)
                    # The subgoal has been achieved
                    else:
                        self.append_transitions_rb(env, o, hindsight_u, ag, self.g, obs, achieved_goals, acts, goals, info_is_sgtt, self.layer_number, goal_status, self.FLAGS.layers-1, is_sgtt=1)
                # It is not a subgoal testing transition
                else:
                    self.append_transitions_rb(env, o, hindsight_u, ag, self.g, obs, achieved_goals, acts, goals, info_is_sgtt, self.layer_number, goal_status, self.FLAGS.layers-1, is_sgtt=0)
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.layer_number) or agent.steps_taken >= env.max_actions or attempts_made >= self.time_limit:
                ...
                if self.layer_number == agent.FLAGS.layers-1 and self.layer_number > 0:
    ...
    def learn(self, num_updates):
        ...
        if self.layer_number == 0:
            ...
            if self.hparams["use_rb"] == "all" or self.hparams["use_rb"] == "lowest":
                if True:
                    print("learning layer 0 with replay_buffer!")
                ...
                if True:
                    print("learning layer 0 with experience_buffer!")
            ...
            # Update nets if replay buffer is large enough
            if self.hparams["use_rb"] == "all":
                if True:
                    print("learning layer 1 with replay_buffer!")
                if self.policy.buffer.get_current_size() >= 50:
                    # Update main nets num_updates times
                    for _ in range(num_updates):
                        self.policy.train()
                ...
                if True:
                    print("learning layer 1 with experience_buffer!")
                if self.replay_buffer.size >= self.batch_size:
                    # Update main nets num_updates times
                    for _ in range(num_updates):
                        self.policy.train()
