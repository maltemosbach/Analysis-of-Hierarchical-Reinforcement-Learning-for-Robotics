...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        self.aAndc = ActorAndCritic(self.sess, self.writer, env, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers)        
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        while True:
            ...
            if self.FLAGS.Q_values:
                ...
                print("Layer %d Q-Value: " % self.layer_number, self.aAndc.get_Q_value(np.reshape(self.current_state,(1,len(self.current_state))), np.reshape(self.goal,(1,len(self.goal))), np.reshape(action,(1,len(action)))))
                if self.layer_number == 2:
                    test_action = np.copy(action)
                    test_action[:3] = self.goal
                    print("Layer %d Goal Q-Value: " % self.layer_number, self.aAndc.get_Q_value(np.reshape(self.current_state,(1,len(self.current_state))), np.reshape(self.goal,(1,len(self.goal))), np.reshape(test_action,(1,len(test_action)))))
