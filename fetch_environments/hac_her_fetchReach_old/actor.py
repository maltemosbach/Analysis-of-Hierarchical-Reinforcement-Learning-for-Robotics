...
class Actor():
    ...
    def __init__(self,
        ...
        self.batch_size = tf.compat.v1.placeholder(tf.float32)

        self.state_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, self.state_dim))
        ...
        self.infer = self.create_nn(self.features_ph, name=self.actor_name + '_main')

        # Target network code "repurposed" from Patrick Emani :^)
        self.weights = [v for v in tf.compat.v1.trainable_variables() if self.actor_name in v.op.name]
        # self.num_weights = len(self.weights)

        # Create target actor network
        self.target = self.create_nn(self.features_ph, name = self.actor_name + '_target')
        self.target_weights = [v for v in tf.compat.v1.trainable_variables() if self.actor_name in v.op.name][len(self.weights):]
        ...
        self.policy_gradient = list(map(lambda x: tf.math.divide(x, self.batch_size), self.unnormalized_actor_gradients))

        # self.policy_gradient = tf.gradients(self.infer, self.weights, -self.action_derivs)
        self.train = tf.compat.v1.train.AdamOptimizer(learning_rate).apply_gradients(zip(self.policy_gradient, self.weights))
    ...
    def create_nn(self, features, name=None):
        ...
        self.max_u = self.action_space_bounds
        self.dimu = 4
        self.hidden = 256
        self.layers = 3
