...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        if self.layer_number == 0:
            self.policy = TD3(self.sess, self.writer, env, hparams, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, use_replay_buffer=hparams["use_rb"])
        else:
