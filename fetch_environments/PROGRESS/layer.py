...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        if len(hparams["buffer"]) == 2:
            assert hparams["buffer"][1] != "replay", "Replay buffer must only be used for the lower layer"


        if hparams["modules"][self.layer_number] == "ddpg":
            self.policy = DDPG(self.sess, env, hparams, self.batch_size, self.experience_buffer, self.replay_buffer, self.transitions_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, buffer_type=hparams["buffer"][self.layer_number], action_l2=self.action_l2)
            self.critic = None
            self.actor = None
    ...
    def append_transition_to_episode(self, env, o, u, ag, g, obs, achieved_goals, acts, goals, layer_number, goal_status, highest_layer):
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        while True:
            ...
            if not agent.FLAGS.test:
                ...
                self.append_transition_to_episode(env, o, hindsight_u, ag, self.g, obs, achieved_goals, acts, goals, self.layer_number, goal_status, self.hparams["layers"]-1)
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.layer_number) or agent.steps_taken >= env.max_actions or attempts_made >= self.time_limit:
                ...
                if self.return_to_higher_level(max_lay_achieved, agent, env, attempts_made):
                    ...
                    if not agent.FLAGS.test and (self.hparams["buffer"][self.layer_number] == "transitions" or self.hparams["buffer"][self.layer_number] == "replay"):
                        obs.append(o.copy())
                        achieved_goals.append(ag.copy())
