...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, writer, agent_params, hparams):
        ...
        if hparams["algo"] == "ddpg":
            ...
            self.policy = TD3(self.sess, self.writer, env, hparams, self.batch_size, self.replay_buffer, self.layer_number, FLAGS, self.hidden, self.layers, self.time_limit, use_replay_buffer=hparams["use_rb"], action_l2=self.action_l2)  
        else:
            assert False
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        while True:
            ...
            #if self.layer_number == 1:
            #    print("Subgoal achieved instead (hindsight_u):", hindsight_u)
