...
def design_agent_and_env(FLAGS, writer, writer_graph, sess, hparams):
    ...
    FLAGS.time_scale = 16    # Enter max sequence length in which each policy will specialize
    ...
    project_state_to_subgoal = lambda sim, state: np.array([1.55 if state[0] > 1.55 else 1.05 if state[0] < 1.05 else state[0], 1.1 if state[1] > 1.1 else 0.4 if state[1] < 0.4 else state[1], 0.7 if state[2] > 0.7 else 0.4 if state[2] < 0.4 else state[2]])
