    ...
    def log_tb(self, step):
        ...
        if log_loss:
            ...
            if self.FLAGS.layers > 1:
                self.writer.add_histogram('layer_1_actor_loss', self.layers[1].policy.actor_loss, step)
                self.writer.add_scalar('layer_1_critic_loss', self.layers[1].policy.critic_loss, step)
                self.writer.add_scalar('layer_1_Q_1_loss', self.layers[1].policy.Q_1_loss, step)
                self.writer.add_scalar('layer_1_Q_2_loss', self.layers[1].policy.Q_2_loss, step)
                if self.FLAGS.layers > 2:
                    self.writer.add_histogram('layer_2_actor_loss', self.layers[2].policy.actor_loss, step)
                    self.writer.add_scalar('layer_2_critic_loss', self.layers[2].policy.critic_loss, step)
                    self.writer.add_scalar('layer_2_Q_1_loss', self.layers[2].policy.Q_1_loss, step)
                    self.writer.add_scalar('layer_2_Q_2_loss', self.layers[2].policy.Q_2_loss, step)
